...
export function AppShell() {
  ...
  return <div {...rootProps}>
    ...
    <main ref={workspaceRef} className={`workspace workspace-${layout} workspace-view-${view}`}>
      ...
      {view === "session" && (!session.connectionError || session.state.updated_at !== null) && layout === "race" && <RaceView state={session.state} analytics={session.analytics} replayAvailable={dataAvailable} intervalsAvailable={session.capabilities?.capabilities.intervals ?? false} positionMode={positionMode} viewingMode={session.viewingMode} layout={preferences.raceLayout} onLayoutChange={preferences.setRaceLayout} onOpenLayoutEditor={openLayoutEditor} onSelectDriver={openDriver} towerView={preferences.towerView} onTowerViewChange={preferences.setTowerView} onOpenStrategy={() => setView("strategy")} />}
      {view === "session" && (!session.connectionError || session.state.updated_at !== null) && layout === "qualifying" && <QualifyingView state={session.state} analytics={session.analytics} sessionKind={classification.kind} replayAvailable={dataAvailable} positionMode={positionMode} viewingMode={session.viewingMode} sectorTimingAvailable={sectorTimingAvailable} onSelectDriver={openDriver} />}
      {view === "session" && (!session.connectionError || session.state.updated_at !== null) && layout === "practice" && <PracticeView state={session.state} replayAvailable={dataAvailable} positionMode={positionMode} viewingMode={session.viewingMode} onSelectDriver={openDriver} />}
