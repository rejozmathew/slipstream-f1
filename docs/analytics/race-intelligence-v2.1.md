...
Official pre-race information is `OfficialPreRaceContext`, separately attributed with source, publication/retrieval time, URL, acquisition mode, caveats, and target-session ownership. Pirelli records come from the bundled normalized seed, quiet historical self-backfill, or sparse current/near-weekend official acquisition. Each path uses the same deterministic extraction, target/session scoping, cutoff/provenance validation, and evidence store; no sample values or manual product transcription are substituted.
