...
`UNAVAILABLE` means the public source cannot currently provide the session. A scheduled session may be pre-event while the source is disconnected; a connected source may also be between meaningful timing updates. These conditions stay separate.
