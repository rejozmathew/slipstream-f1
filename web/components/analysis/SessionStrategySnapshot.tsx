...
export function SessionStrategySnapshot({ analytics, onOpenStrategy, compact = false }: SessionStrategySnapshotProps) {
  ...
  const pirelliContent = baseline?.status === "PRESENT"
    ? <><PirelliNomination baseline={baseline} />{baseline.options.length
      ? <div className="session-published-options">{baseline.options.slice(0, compact ? 1 : 2).map((option) => <PublishedOptionCard key={option.id} option={option} compact />)}</div>
      : <div className="session-pirelli-context-only"><strong>{NO_SPECIFIC_PIRELLI_STRATEGY}</strong>{contextFact && <p><span>{contextFact.category.replaceAll("_", " ")}</span>{contextFact.statement}</p>}</div>}</>
