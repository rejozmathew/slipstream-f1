...
export function AppShell() {
  ...
  if (view === "tv") return <div {...rootProps}><TVModeView state={session.state} sessionLayout={layout} replayAvailable={replayAvailable} positionMode={positionMode} onExit={() => setView("session")} /></div>;
  ...
  return <div {...rootProps}>
    <header className="app-header">
      ...
      <nav aria-label="Product navigation">
        <button className={view === "session" || view === "driver" ? "active" : ""} onClick={() => setView("session")}>SESSION</button>
    ...
    <main className={`workspace workspace-${layout} workspace-view-${view}`}>
      ...
      {view === "session" && !session.connectionError && layout === "race" && <RaceView state={session.state} replayAvailable={replayAvailable} positionMode={positionMode} layout={preferences.raceLayout} onLayoutChange={preferences.setRaceLayout} onOpenLayoutEditor={openLayoutEditor} onSelectDriver={openDriver} />}
      {view === "session" && !session.connectionError && layout === "qualifying" && <QualifyingView state={session.state} replayAvailable={replayAvailable} positionMode={positionMode} onSelectDriver={openDriver} />}
      ...
      {view === "battle" && !session.connectionError && <BattleView state={session.state} stateHistory={session.stateHistory} />}
      {view === "driver" && !session.connectionError && focusedDriver && <DriverFocusView state={session.state} driverNumber={focusedDriver} history={driverHistory.history} historyError={driverHistory.error} playhead={session.playhead} positionMode={positionMode} onBack={() => setView("session")} />}
      {view === "settings" && <SettingsView appearance={preferences.appearance} onAppearanceChange={preferences.setAppearance} raceLayout={preferences.raceLayout} onRaceLayoutChange={preferences.setRaceLayout} section={settingsSection} onSectionChange={setSettingsSection} />}
    </main>
