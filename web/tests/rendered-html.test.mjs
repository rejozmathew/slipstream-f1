...
test("builds the replay-driven desktop shell as static files", async () => {
  ...
  assert.match(bundle, /SEASON \/ WEEKEND \/ SESSION/);
  ...
  assert.match(bundle, /REPRESENTATIVE PACE/);
...
test("keeps versioned API and WebSocket transport in typed clients", async () => {
  const [apiClient, socketClient, page, packageJson, viteConfig] = await Promise.all([
