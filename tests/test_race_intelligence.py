...
def _lap(lap: int, age: int, *, compound: str = "HARD", pit_in: bool = False, duration: float = 90.0) -> LapObservation:
...
def _state(*, track_status: str = "GREEN") -> RaceState:
    return RaceState(
        ...
        drivers={
            "1": DriverState(number="1", code="AAA", position=1, lap=60, compound="HARD", tyre_age=20, status="RUNNING"),
            "2": DriverState(number="2", code="BBB", position=2, lap=60, compound="MEDIUM", tyre_age=10, status="RUNNING", interval_to_ahead="2.0"),
        },
    )
...
def test_to_finish_requires_positive_same_race_evidence() -> None:
    ...
    insufficient = finish_assessment(driver, state, {"1": _finish_evidence()["1"]}, pace, "SATISFIED")
...
def test_generic_yellow_does_not_reset_but_safety_car_does() -> None:
    ...
    assert finish_assessment(yellow.drivers["1"], yellow, _finish_evidence(), pace, "SATISFIED")["canFinish"] is True
    safety_car = _state(track_status="SAFETY CAR")
    assert finish_assessment(safety_car.drivers["1"], safety_car, _finish_evidence(), pace, "SATISFIED")["canFinish"] is None
    ...
    state = replace(
        ...
        drivers={
            **state.drivers,
            ...
            "3": DriverState(number="3", compound="MEDIUM", status="RETIRED", position=3),
        },
    )
    evidence = {
...
def test_race_read_is_structured_and_uses_current_race_pace_only() -> None:
    ...
    models = {
        ...
        "2": {"pace": {"paceTrend": metric(None, status="UNKNOWN", evidence=["missing"])}},
    }
    result = race_read(
        state,
        models,
...
def test_projection_gate_blocks_unstable_or_invalid_future() -> None:
    ...
    strategy = {
        ...
        "primaryStrategy": metric("HARD → SOFT", status="ESTIMATE", evidence=["sample"]),
    ...
    gate = _projection_gate(strategy, state, "LIVE_OUTLOOK", driver=state.drivers["1"], evidence_by_driver={"1": ()})
...
def test_battle_rejects_non_meaningful_large_gap() -> None:
    state = _state()
    drivers = [state.drivers["1"], replace(state.drivers["2"], interval_to_ahead="50.0")]
    models = {
        driver.number: {
            "pace": {"paceTrend": {"value": None}, "degradation": {"value": None}, "currentStintBaseline": None},
            "strategy": {"pitWindow": {"value": None}},
        }
        for driver in drivers
    }
    assert battle_recommendation(drivers, models, layout_family="race")["recommended"] is None


def test_completed_gap_history_is_lap_scoped_and_request_order_independent(tmp_path: Path) -> None:
    events = (
        NormalizedEvent("session", "2026-08-01T14:00:00+00:00", "test", {"key": "300", "session_kind": "race", "layout_family": "race", "status": "STARTED", "lap": 1, "total_laps": 70}),
        NormalizedEvent("driver", "2026-08-01T14:00:01+00:00", "test", {"number": "1", "position": 1, "status": "RUNNING"}),
        NormalizedEvent("timing", "2026-08-01T14:01:00+00:00", "test", {"number": "2", "position": 2, "status": "RUNNING", "interval_to_ahead": "2.0", "lap_observation": {"lap": 1, "started_at": "2026-08-01T14:01:00+00:00", "duration": 90.0, "compound": "MEDIUM", "tyre_age": 1, "quality": "representative"}}),
        NormalizedEvent("timing", "2026-08-01T14:02:30+00:00", "test", {"number": "2", "position": 2, "status": "RUNNING", "interval_to_ahead": "1.8", "lap_observation": {"lap": 2, "started_at": "2026-08-01T14:02:30+00:00", "duration": 90.0, "compound": "MEDIUM", "tyre_age": 2, "quality": "representative"}}),
    )
    evidence = SessionEvidence.from_events(events)
    assert [item.gap_seconds for item in evidence.completed_gap_history("1", "2")] == [2.0, 1.8]
    state = _state()
    state = replace(state, session=replace(state.session, lap=2), drivers={
        "1": replace(state.drivers["1"], lap=2, compound="HARD", tyre_age=2),
        "2": replace(state.drivers["2"], lap=2, compound="MEDIUM", tyre_age=2, interval_to_ahead="1.8"),
    })
    descriptor = SessionDescriptor(key="300", year=2026, meeting_key="30", meeting_name="Test GP", session_name="Race", session_type="Race", circuit="Ring", location="Here", date_start="2026-08-01T14:00:00+00:00", date_end="2026-08-01T16:00:00+00:00", gmt_offset="00:00:00", path=tmp_path / "race.json", source="test", capabilities={})
    resource = ReplayResource(descriptor, events, state, evidence, True, False)
    context = ContextAvailability("unavailable")
    direct = AnalyticsService().snapshot(resource, state, sequence=4, as_of=events[-1].occurred_at, context=context)
    reordered_service = AnalyticsService()
    reordered_service.snapshot(resource, state, sequence=2, as_of=events[1].occurred_at, context=context)
    reordered = reordered_service.snapshot(resource, state, sequence=4, as_of=events[-1].occurred_at, context=context)
    assert direct["battle"]["heldRecommendation"] == reordered["battle"]["heldRecommendation"]
