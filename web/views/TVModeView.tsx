...
import { DriverPirelliContext, PirelliBaseline, RaceNow } from "../components/analysis/PublishedStrategy";
...
import { compoundCode, driverPublishedRoutesText, driverPublishedWindowsText, driverStrategyRelationship } from "../domain/pirelliPresentation.mjs";
...
function DriverTV({ driver, analytics, state, positionMode }: {
  ...
  return <div className="tv-driver-state">
    ...
    <div className="tv-driver-zones">
      ...
      <div className="tv-driver-analysis"><section className="tv-driver-pace"><h3>PACE TREND</h3><PaceDeltaChart samples={samples} compact serverScale={model?.pace.scale} /></section><DriverPirelliContext analytics={analytics} driver={driver} pitEvents={model?.pitEvents ?? []} compact /></div>
    </div>
  </div>;
}
...
function BattleCard({ driver, analytics }: { driver: Driver; analytics: AnalyticsSnapshot | null }) {
  ...
  const observedPath = published?.observedCompounds.map(compoundCode).join(" → ") || "—";
  return <section className="tv-battle-card" style={{ "--team": `#${driver.team_colour ?? "77808f"}` } as React.CSSProperties}><header><b>P{driver.position ?? "—"}</b><strong>{driver.code ?? driver.number}</strong><span>#{driver.number}</span></header><p>{driver.name ?? "Driver"} · {driver.team ?? "Team unavailable"}</p><div><span>TYRE / AGE · STOPS</span><strong><CompoundBadge compound={driver.compound} compact /> {driver.tyre_age == null ? "—" : `${driver.tyre_age}L`} · {driver.pit_count}</strong></div><div><span>OBSERVED PATH</span><strong>{observedPath}</strong></div>{showPublished && <><div><span>PUBLISHED ROUTE</span><strong>{driverPublishedRoutesText(baseline, published)}</strong><small>{driverStrategyRelationship(baseline, published)}</small></div><div><span>STOP WINDOW</span><strong>{driverPublishedWindowsText(baseline, published, false, analytics?.drivers[driver.number]?.pitEvents ?? [])}</strong></div></>}</section>;
}
