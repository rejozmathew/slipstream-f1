...
Protected GPS, high-frequency car data, team radio, and similar enhanced topics remain excluded. Public Live can expose `timing_estimate` when TimingData supplies a complete three-sector mini-sector inventory and known progress statuses. Lap counts without that inventory, partial inventories, reset-only patches, and unknown statuses cannot establish position. Estimates map timing progress approximately onto the separately catalogued circuit; `location_xy` stays false. The cropped authentic public initial fixture has no mini-sectors and remains negative evidence. Representative public-format sparse patches exercise adapter → normalized events → driver track position → cursor-scoped Live mode → map. A bounded public connection probe on 2026-09-04 returned HTTP 403, so current real-live mini-sector availability was not verified; support is conditional on received evidence.
