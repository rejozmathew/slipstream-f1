...
The production artifact for this release declares the 2017–2026 horizon and contains 69 meeting entries / 218 normalized releases. Its source-backed Race baseline is materialized for 66 of the 93 Race descriptors returned by the private metadata source (70.96774193548387%): 9 meetings have an explicit strategy, 46 have compound selection only, and 11 have useful context only. All 66 admitted Race baselines are `STRICT_MODEL`; none require the display-only tier. Three additional normalized meeting entries contain only post-race evidence and therefore correctly fail the Race cutoff. The metadata provider returned no Race descriptors for 2017–2022, so those seasons remain an explicit metadata-source gap rather than fabricated coverage. The complete release report and remaining 2023–2026 gap classification are recorded in [Pirelli V1 closure](pirelli-v1-closure.md).
...
The product-facing `actualStrategy` instead starts from normalized pit events at the inclusive cursor. It appends every factual new compound, including a repeat, so sequences such as `S → S` and `M → M → H` preserve the actual stops, and records each actual stop lap. `completedStops` mirrors canonical state, `observedStops` counts available normalized pit events, `stopLaps` records their factual laps, and `evidenceComplete` is true only when those facts reconcile. The first compound comes from the first event's previous compound or earliest lap evidence; current state fills only the final evidenced-but-missing compound. Missing or conflicting evidence stays incomplete and produces `UNKNOWN` rather than a fabricated sequence.
...
| P28 | Offline older-normalizer → adapted.6 repair preserves the old derivation and builds/imports a current non-empty seed |
| P29 | Exact 69-meeting/218-release bundled manifest, semantic sidebar-contamination guard, deterministic compressed hash, multi-season representatives, and seed-only clean-install API acceptance for Dutch 2026 and Canada 2026 |
...
| P32 | Miami 2026 deliberately removed from a production-seed copy, then immediately prioritized for self-backfill from `FETCHING` to `PRESENT` without restart, with honest client polling termination |
