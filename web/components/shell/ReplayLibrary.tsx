...
export function ReplayLibrary({ catalog, selected, selectedKey, downloadState, downloadError, onSelect, onDownload }: ReplayLibraryProps) {
  ...
  return (
    <details className="replay-library" aria-label="Replay library">
      <summary>
        <span>REPLAY</span>
        <strong>{selected ? `${selected.year} / ${selected.meetingName}` : "SESSION LIBRARY"}</strong>
        <em>{selected?.sessionName ?? "SELECT SESSION"}{selected?.isLive ? " - LIVE" : selected?.available ? " - LOCAL" : " - NOT DOWNLOADED"}</em>
        <i aria-hidden="true" />
      </summary>
      <div className="replay-popover">
        <header><strong>SELECT REPLAY SESSION</strong><span>SEASON / WEEKEND / SESSION</span></header>
