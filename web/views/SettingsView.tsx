...
function PendingSettings({ section }: { section: SettingsSection }) {
  const copy = {
    ...
    preferences: ["Preferences", "Landing session, Battle mode, replay, and notification defaults will be saved to a Viewer Profile."],
...
export function SettingsView({ appearance, onAppearanceChange, raceLayout, onRaceLayoutChange, section, onSectionChange }: {
  ...
  return <div className="settings-shell">
    ...
    <div className="settings-content">{section === "appearance" ? <AppearanceSettings value={appearance} onChange={onAppearanceChange} /> : section === "layouts" ? <LayoutEditor value={raceLayout} onChange={onRaceLayoutChange} /> : <PendingSettings section={section} />}</div>
  </div>;
}
