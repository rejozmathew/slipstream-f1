...
`ReplayLibrary` coalesces loads and preparation, keeps private viewer controllers, and pins resources used by connected replay viewers. Its LRU defaults to three resources within a conservative 512 MiB reservation budget, with a 300 MiB per-resource admission ceiling, including queued and running preparation leases. Asynchronous per-viewer advance and snapshot ownership are cancellation-safe. A full pinned cache rejects another resource with a retryable error. Reservations estimate retained objects and preparation space, not process RSS; allocator/transient memory must be measured separately. No disk preparation format is introduced. Background live identity discovery does not displace prepared historical resources. Deletion of an active session or a target with active/pending publication returns HTTP 409 Conflict.
...
Transport status (`OFFLINE`, `CONNECTING`, `LIVE`, `STALE`, `UNAVAILABLE`) is internal evidence. The authoritative product lifecycle is `PRE_EVENT`, `CONNECTING`, `LIVE`, `STALE`, `RECONNECTING`, `FINALIZING`, `COMPLETE`, `REPLAY_READY`, or `UNAVAILABLE`. A disconnect never completes a session. Only explicit source completion evidence begins `FINALIZING`; every newly emitted canonical factual event extends a deterministic drain, while Heartbeat/no-op rows do not. The single live collector releases immediately after factual drain, allowing the next live target to progress while publication of the old session retries independently with bounded backoff and retained delayed tails. The normalized recorder atomically renames its in-progress artifact into the ordinary ReplayLibrary format, and catalog invalidation makes it `REPLAY_READY`. On restart, unfinished recent journals are validated for live recovery, and completed journals are separately published offline without requiring an upstream connection. Attempting to delete an active session or a target with pending/active publication returns HTTP 409.

At zero delay, `PublicLiveSession` serves its incrementally maintained `state` and `evidence`; it does not rebuild the full event history for every snapshot. Delayed viewers reconstruct at their private cursor. The live WebSocket keeps one shared immutable event history and a private delay per viewer (0–300 seconds, with presets 5s, 10s, 30s, 1m, 2m, 3m, 5m and exact M:SS), and state plus analytics always share the same inclusive event sequence. A viewer can reset to live or change delay but cannot pause, seek backward, or alter playback speed. `FINALIZING` retains the same session's last authoritative Live state while accepted late canonical events drain. Delayed viewers consume the old session's remaining tail before receiving the finalized ReplayLibrary state handoff. A deliberate session selection stays with the selected session, while follow-live viewers receive `live.nextSessionKey` after draining their tail and can advance to the new live target. Raw versioned JSONL remains optional provider evidence and is not the replay artifact.
